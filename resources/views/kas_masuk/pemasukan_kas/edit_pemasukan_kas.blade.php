          ...
          <div class="row" style="margin-bottom: 50px">
            <div class="col-sm-6 table-responsive" >
              ...
              <table class="table header_petty">
                  ...
                  <td width="30%">NOTA</td>
                  <td>
                    <input value="{{ $data->km_nota }}" type="text" readonly="" value="" class="km_nota form-control wajib" name="km_nota">
                  </td>
                </tr>
                    ...
                    <td>
                      <select class="km_sekolah form-control option " name="km_sekolah" onchange="nota()">
                        ...
                        @foreach($sekolah as $val)
                          <option @if ($data->km_sekolah == $val->s_id)
                            selected="" 
                ...
                <tr>
                  <td>TANGGAL</td>
                  <td>
                    <input type="text" readonly="" value="{{ carbon\carbon::parse($data->km_tanggal)->format('d-m-Y') }}" class="pc_tanggal form-control" name="pc_tanggal">
                  </td>
                </tr>
                <tr>
                  <td>KETERANGAN</td>
                  <td>
                    <input type="text" value="{{ $data->km_keterangan }}" class="km_keterangan huruf_besar form-control wajib" name="km_keterangan">
                  </td>
                </tr>
                  <tr>
                    <td>AKUN KAS</td>
                    <td>
                      <select class="km_akun_kas form-control option" name="km_akun_kas">
                        ...
                        @foreach($akun_kas as $val)
                          <option @if ($data->km_akun_kas == $val->a_master_akun)
                            selected=""
...
$(document).ready(function(){
  ...
  $('.km_total').maskMoney('mask','{{ $data->km_total }}');
})
...
$('.simpan').click(function(){
  ...
  $.ajax({
     type: "POST",
     url: baseUrl +'/kas_masuk/update_pemasukan_kas',
     data:$('.header_petty :input').serialize()+'&'+
          data_petty.$('input').serialize()+'&id='+'{{ $data->km_id }}',
     dataType:'json',
...
@foreach ($data->kas_masuk_detail as $i => $k)
  var pcd_akun_biaya   = '{{ $k->kmd_akun_pendapatan }}';
  @foreach ($akun as $a => $akun)
    @if ($k->kmd_akun_pendapatan == $akun->a_master_akun)
      var pcd_akun_biaya_t = '{{ $akun->a_master_nama }}';
    @endif
  @endforeach
  var kmd_keterangan   = '{{ $k->kmd_keterangan }}';
  var pcd_jumlah       = '{{ $k->kmd_total }}';
  data_petty.row.add([
    '<p class="pcd_nama_biaya_text">'+pcd_akun_biaya_t+'</p>'+
    ' <input type="hidden" class="pcd_akun_biaya" name="pcd_akun_biaya[]" value="'+pcd_akun_biaya+'">'+
    ' <input type="hidden" class="indexs indexs_'+indexs+'">',
    ...
    ' <input type="text" readonly class="pcd_jumlah form-control right" value="'+accounting.formatMoney(pcd_jumlah,"", 0, ".",',')+'" name="kmd_total[]">',
