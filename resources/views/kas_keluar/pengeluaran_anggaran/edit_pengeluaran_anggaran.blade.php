    ...
    <div class="col-lg-12 grid-margin stretch-card">
      <div class="card">
        <div class="card-body">
          ...
          <div class="row" style="margin-bottom: 50px">
            ...
            <div class="col-sm-6 table-responsive">
              ...
              <table class="table input_data">
                <tr>
                  <td>
                    <input type="text"  id="kode_rencana" name="kode_rencana" readonly="" placeholder="Klik Untuk Memilih Rencana Pembelian" class="form-control wajib" value="{{ $data->pc_ref }}">
                  </td>
                </tr>
              </table>
            </div>
          ...
          <hr>
          <div class="row div_barang clearfix">
...
$('.simpan').click(function(){
  ...
  $.ajax({
     type: "POST",
     url: baseUrl +'/kas_keluar/update_pengeluaran_anggaran',
...
@foreach ($data->rencana_pembelian->rencana_pembelian_d as $val)
  var nama_barang     = '{{ $val->barang->b_nama }}';
  var rpd_detail      = '{{ $val->rpd_detail }}';
  var rpd_barang      = '{{ $val->rpd_barang }}';
  var rpd_sisa        = '{{ $val->rpd_sisa }}'*1;
  var harga_barang    = '{{ $val->barang->b_harga_tertinggi }}';
  var pcd_jumlah      = '';
  var pcd_qty         = '';
  var pcd_total       = '';
  var pcd_keterangan  = '';
  @foreach ($data->petty_cash_detail as $val1)
    @if ($val1->pcd_rpd_detail == $val->rpd_detail)
      pcd_jumlah = '{{ $val1->pcd_jumlah }}';
      pcd_qty    = '{{ $val1->pcd_qty }}'*1;
      pcd_total  = pcd_jumlah * pcd_qty;
      pcd_keterangan  = '{{ $val1->pcd_keterangan }}';
      rpd_sisa+=pcd_qty;
    @endif
  @endforeach
  tables.row.add([
            '<p class="pcd_nama_biaya_text">'+nama_barang+'</p>'+
            '<input type="hidden" class="rpd_detail" name="rpd_detail[]" value="'+rpd_detail+'">'+
            '<input type="hidden" class="rpd_barang" name="rpd_barang[]" value="'+rpd_barang +'">',

            '<p class="pcd_sisa_text" align="right">'+rpd_sisa+'</p>'+
            '<input type="hidden" class="pcd_sisa" name="pcd_sisa[]" value="'+rpd_sisa +'">',

            '<p class="pcd_harga_tertinggi_text" align="right">'+accounting.formatMoney(harga_barang ,"", 0, ".",',')+'</p>'+
            '<input type="hidden" class="pcd_harga_tertinggi" name="pcd_harga_tertinggi[]" value="'+harga_barang +'">',

            '<input type="text" class="pcd_harga form-control mask right" name="pcd_jumlah[]" value="'+accounting.formatMoney(pcd_jumlah ,"", 0, ".",',')+'">',

            '<input type="text" class="hanya_angka pcd_jumlah form-control right" value="'+pcd_qty+'" name="pcd_qty[]">',

            '<input type="text" readonly class="pcd_total form-control right" value="'+accounting.formatMoney(pcd_total ,"", 0, ".",',')+'" name="pcd_total[]">',

            '<input type="text" class="pcd_keterangan form-control right" value="'+pcd_keterangan+'" name="pcd_keterangan[]">',
